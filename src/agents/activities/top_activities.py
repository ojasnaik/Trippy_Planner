...
async def get_top_activities(ctx: Context, sender: str, msg: UAgentResponse):
    ...
    prompt = f"""You are an expert AI in suggesting travel. The following is the conversation that you have had with the user: 
    {msg.message}. 
    Based on this suggest some activites that you can do in that city, keeping in mind the constraints and discussion you have had with the till now(provided in the prompt above).
"""
    try:
